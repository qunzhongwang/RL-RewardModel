...
# #dist.init_process_group(backend="nccl")  # 或 "gloo" 等后端
...
def main():
    ...
    accelerator_config = ProjectConfiguration(
        project_dir=os.path.join("logs","tst"),#config.logdir, config.run_name),
        automatic_checkpoint_naming=True,
        total_limit=5,#config.num_checkpoint_limit,
    )

    accelerator = Accelerator(
        log_with="wandb",
        mixed_precision="bf16",#config.mixed_precision,
        project_config=accelerator_config,
        gradient_accumulation_steps=1#config.train.gradient_accumulation_steps,
    )
    ...
    PROJECT_NAME = "Qwen_RL"
    if accelerator.is_main_process:
        accelerator.init_trackers(
            project_name="DEBUG",
            config={},
            init_kwargs={"wandb": {"name": "1"}},#}config.run_name}},
        )
