...
def main(_):
    ...
    if config.resume_ckpt:
        lora_path = find_ckpt(config.loradir, config.resume_ckpt,mode="lora")
        ...
        checkpoint_path = find_ckpt(config.logdir, config.resume_ckpt, mode="ckpt")
    ...
    for key1,key2 in zip(cumulative_sums, cumulative_counts):
        ...
        if isinstance(cumulative_counts[key2], torch.Tensor):
            cumulative_sums[key2] = cumulative_sums[key2].item()
    ...
    for epoch in tqdm(range(init_epoch, config.num_epochs)):
        ...
        if accelerator.is_main_process and not config.inference:  # 仅主进程保存
            ...
            checkpoint_path = os.path.join(config.logdir, f"checkpoint_epoch_{ckptuid}_{epoch}.pt")
            accelerator.save({
                "epoch": epoch + 1,
            ...
            lora_pth = os.path.join(config.loradir,f"lora_epoch_{ckptuid}_{epoch}")
